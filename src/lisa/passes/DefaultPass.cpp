...
namespace lisa::passes {
  DefaultPass::DefaultPass(const str& name) : RenderPass(name) {
    ...
    const auto shader = resources::context::manager().load<resources::Shader>("shader");
  ...
  void DefaultPass::render(
    ...
    in.cmd_buffer->bindPipeline(vk::PipelineBindPoint::eGraphics, **pipeline_);
    ...
    uint32 i = 0;
    for (const auto& entity : in.scene.entities()) {
      const auto mesh = resources::context::manager().get<resources::Mesh>(entity.mesh_id);
      const auto texture =
        resources::context::manager().get<resources::Texture>(entity.texture_id);
