...
private:
  ...
  void initWindow() {
    ...
    window =
      glfwCreateWindow(WIDTH, HEIGHT, "Vulkan Profiles Demo", nullptr, nullptr);
  ...
  void initVulkan() {
    ...
    if (!appInfo.profileSupported) createRenderPass();

    createDescriptorSetLayout();
    createGraphicsPipeline();

    // Create framebuffers only if not using dynamic rendering
    if (!appInfo.profileSupported) createFramebuffers();
  ...
  void checkFeatureSupport() {
    ...
    if (result == VK_SUCCESS && supported == VK_TRUE) {
      ...
      // If we wanted to implement fallback, we would call
      // detectFeatureSupport() here But for this example, we'll just use
      // traditional rendering if the profile isn't supported
    }
  }
  ...
  void createLogicalDevice() {
    ...
    if (appInfo.profileSupported) {
      ...
      std::cout << "Created logical device using KHR roadmap 2022 profile"
                << std::endl;
    } else {
  ...
  void createImageViews() {
    ...
    for (const auto& image : swapChainImages) {
      swapChainImageViews.push_back(createImageView(
        image, swapChainSurfaceFormat.format, vk::ImageAspectFlagBits::eColor, 1
      ));
    }
  }
  ...
  void createGraphicsPipeline() {
    ...
    if (appInfo.profileSupported) {
      std::cout
        << "Creating pipeline with dynamic rendering (KHR roadmap 2022 profile)"
        << std::endl;
    } else {
      std::cout << "Creating pipeline with traditional render pass (fallback)"
                << std::endl;
  ...
  void createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
      std::array<vk::ImageView, 3> attachments = { *colorImageView,
                                                   *depthImageView,
  ...
  void generateMipmaps(
    ...
    vk::ImageMemoryBarrier barrier{ .srcQueueFamilyIndex =
  ...
  void createUniformBuffers() {
    ...
    // Reserve space but don't resize, as RAII objects can't be
    // default-constructed
  ...
  void recordCommandBuffer(uint32_t imageIndex) {
    ...
    // COLOR_ATTACHMENT_OPTIMAL 1) Multisampled color attachment image ->
    ...
    if (appInfo.profileSupported) {
      ...
      vk::RenderingInfo renderingInfo{ .renderArea = { { 0, 0 },
                                                       swapChainExtent },
                                       .layerCount = 1,
                                       .colorAttachmentCount = 1,
  ...
  void drawFrame() {
    vk::Result fenceResult = device.waitForFences(
      { *inFlightFences[frameIndex] }, VK_TRUE, UINT64_MAX
    );
  ...
  SwapChainSupportDetails
    querySwapChainSupport(vk::raii::PhysicalDevice device) {
