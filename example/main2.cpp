...
std::vector<VkImageView> swapchainImageViews; // The views of the images stored by the swapchain
...
VmaAllocation vBufferAllocation{VK_NULL_HANDLE}; // Allocation for the vertex buffer
VkBuffer vBuffer{VK_NULL_HANDLE}; // Vertex buffer
VkDeviceSize vBufSize{0};
VkDeviceSize indexCount{0};

constexpr uint32_t maxFramesInFlight{2}; // The maximum number of frames "in flight" (processed concurrently)
...
void get_logical_device()
{
    ...
    constexpr VkPhysicalDeviceFeatures enabledVk10Features{
...
void add_depth_attachment()
{
    ...
    depthImageCI = {
        ...
        .format = depthFormat,
        .extent{
            .width = static_cast<uint32_t>(windowSize.x), .height = static_cast<uint32_t>(windowSize.y), .depth = 1
        },
        .mipLevels = 1,
        .arrayLayers = 1,
...
void load_obj(const char* filename)
{
    ...
    chk(tinyobj::LoadObj(&attrib, &shapes, &materials, nullptr, nullptr, filename));
...
void load_textures()
{
    for (auto i = 0; i < textures.size(); i++)
    {
        ...
        VkImageViewCreateInfo texVewCI{
            .sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO, .image = textures[i].image,
        ...
        // Copy image from intermediary buffer to GPU using a command buffer and a fence
        ...
        textureDescriptors.push_back({
            .sampler = textures[i].sampler, .imageView = textures[i].view,
...
{
    // Define the interface between our application and the shader in the form of a descriptor set layout
...
void load_shaders()
{
    ...
    VkShaderModuleCreateInfo shaderModuleCI{
        .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, .codeSize = spirv->getBufferSize(),
...
void update_swapchain()
{
    ...
    swapchainCI.imageExtent = {
        .width = static_cast<uint32_t>(windowSize.x), .height = static_cast<uint32_t>(windowSize.y)
    };
    chk(vkCreateSwapchainKHR(device, &swapchainCI, nullptr, &swapchain));
    for (auto i = 0; i < imageCount; i++)
    {
...
void render_loop()
{
    ...
    while (!quit)
    {
        ...
        // Acquire next image
        chkSwapchain(vkAcquireNextImageKHR(device, swapchain, UINT64_MAX, presentSemaphores[frameIndex], VK_NULL_HANDLE,
                                           &imageIndex));
        // Update shader data
