...
std::vector<VkPhysicalDevice> devices; // The list of physical devices available
uint32_t deviceIndex{ 0 }; // The index of the selected physical device
VkDevice device{ VK_NULL_HANDLE }; // The device

uint32_t queueFamily{ 0 }; // The queue family to use
VkQueue queue{ VK_NULL_HANDLE }; // The queue of the device

VmaAllocator allocator{ VK_NULL_HANDLE }; // The memory allocator

VkSurfaceKHR surface{ VK_NULL_HANDLE }; // The surface to "paint" to
VkSurfaceCapabilitiesKHR surfaceCaps{}; // The surface's capabilities
glm::ivec2 windowSize{};

constexpr VkFormat imageFormat{ VK_FORMAT_B8G8R8A8_SRGB };
VkSwapchainCreateInfoKHR swapchainCI; // The swapchain create information
VkSwapchainKHR swapchain{ VK_NULL_HANDLE }; // The swapchain
std::vector<VkImage> swapchainImages; // The images stored by the swapchain
std::vector<VkImageView>
  swapchainImageViews; // The views of the images stored by the swapchain
uint32_t imageCount{ 0 };

VkFormat depthFormat{ VK_FORMAT_UNDEFINED };
VkImageCreateInfo depthImageCI;
VkImage depthImage; // The depth image
VmaAllocation depthImageAllocation; // The memory allocation of the depth image
VkImageView depthImageView; // The view of the depth image

VmaAllocation vBufferAllocation{
  VK_NULL_HANDLE
}; // Allocation for the vertex buffer
VkBuffer vBuffer{ VK_NULL_HANDLE }; // Vertex buffer
VkDeviceSize vBufSize{ 0 };
VkDeviceSize indexCount{ 0 };

constexpr uint32_t maxFramesInFlight{
  2
}; // The maximum number of frames "in flight" (processed concurrently)
...
void get_physical_device(int argc, char* argv[]) {
...
void get_queue_family() {
  uint32_t queueFamilyCount{ 0 };
  vkGetPhysicalDeviceQueueFamilyProperties(
    devices[deviceIndex], &queueFamilyCount, nullptr
  );
...
void get_logical_device() {
  ...
  VkDeviceQueueCreateInfo queueCI{ .sType =
  ...
  constexpr VkPhysicalDeviceFeatures enabledVk10Features{ .samplerAnisotropy =
                                                            VK_TRUE };
...
void add_depth_attachment() {
  // Check the device supports depth attachment
  ...
  for (VkFormat& format : depthFormatList) {
    ...
    vkGetPhysicalDeviceFormatProperties2(
      devices[deviceIndex], format, &formatProperties
    );
    if (
  ...
  // Define the properties of the depth image
  depthImageCI = {
...
void load_obj(const char* filename) {
  ...
  chk(vmaCreateBuffer(
    allocator, &bufferCI, &bufferAllocCI, &vBuffer, &vBufferAllocation, nullptr
  ));
  void* bufferPtr{ nullptr };
  chk(vmaMapMemory(allocator, vBufferAllocation, &bufferPtr));
  memcpy(bufferPtr, vertices.data(), vBufSize);
  memcpy(static_cast<char*>(bufferPtr) + vBufSize, indices.data(), iBufSize);
  vmaUnmapMemory(allocator, vBufferAllocation);
}

void setup_shader_data_buffers() {
  for (auto i = 0; i < maxFramesInFlight; i++) {
    ...
    chk(vmaMapMemory(
      allocator, shaderDataBuffers[i].allocation, &shaderDataBuffers[i].mapped
    ));
...
void setup_fences_and_semaphores() {
  constexpr VkSemaphoreCreateInfo semaphoreCI{
    .sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO
  };
  constexpr VkFenceCreateInfo fenceCI{ .sType =
...
void load_textures() {
  for (auto i = 0; i < textures.size(); i++) {
    ...
    // Create an intermediary buffer to store the image data
    VkBuffer imgSrcBuffer{};
    VmaAllocation imgSrcAllocation{};
    VkBufferCreateInfo imgSrcBufferCI{ .sType =
    ...
    // Copy the image data to the intermediary buffer
    ...
    // Copy image from intermediary buffer to GPU using a command buffer and a
    // fence
    ...
    // Copy the image to the GPU optimally
...
void setup_descriptor_indexing() {
  // Define the interface between our application and the shader in the form of
  ...
  chk(vkCreateDescriptorSetLayout(
    device, &descLayoutTexCI, nullptr, &descriptorSetLayoutTex
  ));
  // Create a descriptor pool
  VkDescriptorPoolSize poolSize{ .type =
...
void load_shaders() {
  ...
  Slang::ComPtr slangModule{ slangSession->loadModuleFromSource(
...
void update_swapchain() {
  ...
  depthImageCI.extent = { .width = static_cast<uint32_t>(windowSize.x),
                          .height = static_cast<uint32_t>(windowSize.y),
                          .depth = 1 };
  VmaAllocationCreateInfo allocCI{ .flags =
...
void render_loop() {
  ...
  while (!quit) {
    ...
    for (auto i = 0; i < 3; i++) {
      auto instancePos =
        glm::vec3(static_cast<float>(i - 1) * 3.0f, 0.0f, 0.0f);
      shaderData.model[i] = glm::translate(glm::mat4(1.0f), instancePos) *
                            glm::mat4_cast(glm::quat(objectRotations[i]));
    }
    memcpy(
      shaderDataBuffers[frameIndex].mapped, &shaderData, sizeof(ShaderData)
    );
    // Record command buffer
    const auto cb = commandBuffers[frameIndex];
    ...
    VkDependencyInfo barrierDependencyInfo{ .sType =
    ...
    for (SDL_Event event; SDL_PollEvent(&event);) {
      ...
      // Window resize
      if (event.type == SDL_EVENT_WINDOW_RESIZED) updateSwapchain = true;
    }

    if (updateSwapchain) update_swapchain();
  }
}
...
void clean_up() {
  ...
  for (auto i = 0; i < maxFramesInFlight; i++) {
    ...
    vmaDestroyBuffer(
      allocator, shaderDataBuffers[i].buffer, shaderDataBuffers[i].allocation
    );
  }
...
int main(int argc, char* argv[]) {
  init_libraries();
  create_instance();
  get_physical_device(argc, argv);
  get_queue_family();
  get_logical_device();
  setup_vma();
  setup_window_and_surface();
  setup_swapchain();
  add_depth_attachment();
  load_obj("../../assets/suzanne.obj");
  setup_shader_data_buffers();
  setup_fences_and_semaphores();
  setup_command_buffers();
  load_textures();
  setup_descriptor_indexing();
  load_shaders();
  setup_pipeline();
  render_loop();
  clean_up();
